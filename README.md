...
|--------|---------|--------------|
| [`issuekit`](./plugins/issuekit/) | 0.2.2 | Shared adapter layer for issue-tracker plugins. Auto-detects the active MCP (Azure DevOps, Jira) and exposes an abstract verb surface (`getIssue`, `createIssue`, `addComment`, `transition`, `linkIssue`, `getSprintItems`, `getTeamCapacity`, ...). Auto-installed as a dependency of `incident-postmortem`, `issue-triage`, `sprint-status-report`, and `draft-stories`. Ships no MCP, no agent, no slash command — it's a library. Bring your own MCPs. |
...
`incident-postmortem`, `issue-triage`, `sprint-status-report`, `draft-stories`, and `acceptance-test-generator` declare `issuekit` as a dependency; Claude Code auto-installs it when you install any of them. Install `issuekit` directly only if you want the shared verb surface for some other purpose.
