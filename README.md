...
| [`sprint-status-report`](./plugins/sprint-status-report/) | 1.0.0 | Reads the current sprint on any supported tracker (Azure DevOps, Jira), tallies work items by state (Done / In Progress / To Do), computes percent complete and remaining count, flags blocked and stale items, then emits a [Marp](https://marp.app/) markdown deck ready to export to PPTX/PDF. Read-only: no writes, no confirmation gate, safe to run anytime. Team capacity is an Azure-DevOps-only slide (Jira has no capacity concept). Bundles `sprint-analyst` and `deck-composer`. Ships `/sprint-status-report:run [sprint] [--team <name>] [--export pptx\|pdf]`. Auto-installs `issuekit`. |
