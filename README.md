...
| [`azure-issue-triage`](./plugins/azure-issue-triage/) | 0.1.0 | Sibling of `jira-issue-triage` for Azure DevOps Boards. Triages Bug and Task work items (v0.1.0 cut-down): assigns, runs the matching investigation skill, refines `System.Title` and `System.Description` (HTML), posts an archetype-appropriate comment, and posts a Microsoft Teams summary. Bundles `azure-issue-investigator`, `azure-requirements-investigator`, `azure-work-item-refiner`, and `prose-style`. Ships a `/azure-issue-triage:setup` wizard. Incident, User Story, Spike, escalation, sprint placement, and SLA-based due dates land in v0.2.0+. |
