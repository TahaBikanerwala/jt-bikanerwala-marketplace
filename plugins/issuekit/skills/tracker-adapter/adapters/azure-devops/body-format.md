...
The canonical authoring format is markdown; the adapter emits HTML. The translation:
...
| `---` (horizontal rule) | `<hr/>` | |
| Tables | `<table>...</table>` with `<thead>`, `<tbody>`, `<tr>`, `<th>`, `<td>` | Plain HTML table. AzDO renders cleanly. Out of canonical subset; surfaces a warning if the input markdown included one. |
| Inline image `![alt](url)` | `<img src="url" alt="alt"/>` | URL must be either an attachment URL on the same work item or an absolute https URL. Hot-linking arbitrary external images works but may break later. Out of canonical subset. |
| `@[userRef]` (mention) | `<a href="#" data-vss-mention="version:2.0,<unique-name>">@Display Name</a>` | `userRef` is resolved via `resolveUser`; the unique-name and display-name are looked up on the descriptor. |
...
## HTML content-loss warning (read side)

The fetched description is HTML. When the adapter reads it for `getIssue`, certain HTML constructs are awkward to round-trip through a markdown rewrite:
...
| `@mentions` of users (rendered as `<a href="..." data-vss-mention="...">@User Name</a>`) | Common | The adapter preserves the mention as `@[userRef]` if `resolveUser` succeeds; otherwise the plain @ + name remains and the link is lost. |
