...
Plug-and-play in this suite = `issuekit` + `postmortem-generator` + your own MCPs. This plugin ships **no** `.mcp.json` and bundles **no** vendor config. The tracker-adapter resolves which tracker is active by inspecting the tools the user has registered.
...
```
@postmortem-generator <incident URL or ID>
```

or the slash-command shorthand:

```
/postmortem-generator:run <incident URL or ID>
```
