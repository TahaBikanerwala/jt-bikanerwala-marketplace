---
description: Read-only. Print the complete bug report the agent would file, including the list of missing information, without creating or updating anything in the tracker.
...
the tracker, or to see what information is still missing from a vague report.
...
3. **Pause** on the clarification card, same as `:run`. Skipping still drops the section and routes the
   gap to Missing Information.
4. Print, with no gate and no writes:
...
No codebase is read, in this mode or in `:run`. For the defect localized in code and a proposed fix,
run `/issue-triager:run` on the bug once it exists.
