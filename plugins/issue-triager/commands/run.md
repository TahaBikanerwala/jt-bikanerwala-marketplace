...
This command is a thin shell. It dispatches to the `issue-triager` agent with the URL/ID as the input. The agent runs the full ten-phase workflow:
...
- `/issue-triager:investigate-and-refine` — lightweight subset that only investigates and refines title/description; no field updates, no transitions, no escalation.
