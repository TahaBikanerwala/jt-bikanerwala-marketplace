...
This skill runs without user interaction. The constraints below let it work cleanly inside the `issue-triager` agent (which has its own confirmation gate) and standalone.
...
1. Identify the issue ID/key from the invocation context. If the calling context (the `issue-triager` agent in Phase 1) has already fetched the issue and exposed the payload, reuse it; don't re-fetch.
