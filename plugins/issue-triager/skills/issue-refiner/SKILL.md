...
This skill **does not write to the tracker**. It returns the refined title and markdown body as plain text. The caller (the `issue-triager` agent in Phase 3 prep, or a user invoking the skill directly with a manual write step) batches the write through the adapter's diff-and-confirm gate.
...
The skill works two ways. When a user pastes an issue and asks to refine it, the skill fetches and refines, then returns. When the `issue-triager` agent calls in Phase 3 prep, the agent has already fetched the payload and the investigation report; skip the fetch step.
...
When the caller (the `issue-triager` agent) has already fetched the issue and exposes the payload, reuse it. Don't re-fetch.
