...
Delta entry point for the `sprint-status-report` agent. Compares the sprint's current state
...
/sprint-status-report:progress-delta --baseline sprint-reports/.snapshots/sprint-42-2026-07-07.json
```

## Behavior

This command is a thin shell. It dispatches to the `sprint-status-report` agent with
...
3. Fetch the sprint's current work items and (Azure DevOps only) team capacity, and compute
   the current metrics (`sprint-analyst`).
...
5. Diff baseline against current (`delta-analyst`).
