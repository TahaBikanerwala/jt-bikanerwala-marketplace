...
Render the delta model from `delta-analyst` into a [Marp](https://marp.app/) markdown deck
...
- Do not compute or re-classify anything; if a number or transition looks off, that is
  `delta-analyst`'s job, not this skill's. Render faithfully.
