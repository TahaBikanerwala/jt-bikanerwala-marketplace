...
about and tested in isolation. It complements `sprint-analyst` (which computes a single
...
```json
{
  ...
  "baseline": {
    ...
    "items":   [ SprintItem, ... ],
    "metrics": <sprint-analyst metrics model>
  },
  "current": {
    "generatedAt": "YYYY-MM-DD",
    "items":   [ SprintItem, ... ],
...
exact output of `sprint-analyst` for each state; reuse their numbers rather than
recomputing progress from scratch.

**Blocked / stale definitions come from `sprint-analyst`.** An item is blocked or stale by
...
`sprint-analyst` (a short plain-text line; `""` when the title says everything).
...
- Do not recompute progress, blocked, or stale from raw items; consume the two
  `sprint-analyst` models. This skill only diffs.
