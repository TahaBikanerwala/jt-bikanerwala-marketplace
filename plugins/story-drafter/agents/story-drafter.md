...
| Phase 1 | `requirements-brainstormer` (this plugin) | PM analysis of the raw input into a topic-by-topic requirement breakdown. |
...
Invoke `requirements-brainstormer` with `Calling context: phase=1.` and `requirements_text` as the payload. Cache the returned topic-by-topic analysis as `brainstorm`. This is a working artifact; it is never saved to disk and never posted to the tracker.
