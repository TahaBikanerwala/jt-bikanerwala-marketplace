...
This skill reads. It never writes, asks, or touches the tracker. It reuses the code-reading ladder and evidence model from `issue-triager`'s `requirements-investigator` (Level 3), applied to a set of questions instead of a single issue.
