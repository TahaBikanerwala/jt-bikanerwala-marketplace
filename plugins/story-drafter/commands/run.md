...
Entry point for the `story-drafter` agent. Pass the raw requirements as the argument, or run it bare and paste them when asked.

## Examples

```
/story-drafter:run We need faculty to be able to share an agent's skills with a whole cohort, and IT wants to control who can publish.
...
This command is a thin shell. It dispatches to the `story-drafter` agent with the input. The agent runs its workflow:

1. Bootstrap the tracker via `issuekit:tracker-adapter` and read the requirements.
2. Brainstorm the requirements topic-by-topic (`requirements-brainstormer`).
...
- `/issue-triager:run` — triage and refine an **existing** issue (this plugin creates **new** ones).
