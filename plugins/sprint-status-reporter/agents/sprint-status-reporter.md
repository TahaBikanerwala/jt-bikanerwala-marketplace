...
The dispatcher passes `mode=status` or `mode=delta` (from `/sprint-status-reporter:run` and
`/sprint-status-reporter:progress-delta` respectively) plus the raw argument string. If `mode`
...
| Phase 3 (both modes) | `sprint-analyzer` (this plugin) | Turn a raw item list + capacity into a metrics model. Delta mode also runs it on the reconstructed baseline set. |
| Phase 4 (status) | `deck-composer` (this plugin) | Render the metrics model into a Marp status deck. |
| Phase D2 (delta) | `delta-analyzer` (this plugin) | Diff the baseline metrics + items against the current ones into a delta model. |
...
| `delta` | Phase D2 | D3 | delta model from `delta-analyzer` |
