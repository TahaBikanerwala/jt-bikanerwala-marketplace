...
Entry point for the `sprint-status-reporter` agent. Runs against whatever tracker MCP is
...
This command is a thin shell. It dispatches to the `sprint-status-reporter` agent with the
...
4. Analyze: bucket by state, compute progress, flag blocked and stale items (`sprint-analyzer`).
...
is what `/sprint-status-reporter:progress-delta` compares against, so running this weekly builds
