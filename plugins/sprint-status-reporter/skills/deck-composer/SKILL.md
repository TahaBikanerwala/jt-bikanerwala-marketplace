...
Render the metrics model from `sprint-analyzer` into a [Marp](https://marp.app/) markdown
...
```json
{
  ...
  "metrics": <the sprint-analyzer metrics model>,
  "today":   "YYYY-MM-DD",
...
- Do not compute or re-bucket anything; if a number looks off, that's `sprint-analyzer`'s
