...
These optional fields default to null or to a sensible built-in: `scope_summary_field_name`, `sprint_field_name`, `story_points_field_name`, `non_bug_transitions.ready`, `archetype_assignment_after_triage`, `description_preview_pause_seconds`. When null, the agent skips the steps that reference them. Two have non-null defaults: `archetype_assignment_after_triage` defaults to the 1.2.0 assignment behavior (Bug → unassign, all others → self) so existing installs see the same Phase 9 outcomes after upgrade; `description_preview_pause_seconds` defaults to 3 seconds, reflecting the new Phase 5 pause-then-write flow introduced in 1.3.0 that replaced the 1.2.0 second confirmation gate. Each field is documented in the plugin README's Advanced Configuration section.
...
3. If `story_points_field_name` is configured AND `story_point_estimate` was cached at the Phase 3 gate (main panel question 3) as a numeric value, write the estimate to the configured field via `editJiraIssue`. If the user picked "Skip" or returned an empty/non-numeric "Other" answer at Phase 3, `story_point_estimate` is `null` and Phase 6 silently leaves the field unwritten. If the field name is configured but the field ID was not resolved during Prerequisites auto-discovery, skip the write silently and note the miss in the Phase 10 DM.
